...
certificatesResolvers:
  letsEncrypt:
    acme:
      email: flomaster@{{lookup('env','prefix')}}.az.skillscloud.company
      storage: acme.json
...
http:
  routers:
    http-catchall:
      rule: Host("app.{{lookup('env','prefix')}}.az.skillscloud.company")
      # rule: hostregexp("{.+}")
      entrypoints:
      - http
      middlewares:
    ...
    web-53:
      ...
      tls: {}
    web-53-01:
      rule: Host("region-02.{{lookup('env','prefix')}}.az.skillscloud.company")
      entrypoints:
      - https
      service: web-53
      tls: {}
    web-53-02:
      rule: Host("region-01.{{lookup('env','prefix')}}.az.skillscloud.company")
      entrypoints:
      - https
      service: web-53
      tls: {}
    web-53-03:
      rule: Host("region-03.{{lookup('env','prefix')}}.az.skillscloud.company")
      entrypoints:
      - https
      service: web-53
      tls: {}
  ...
  services:
    web-53:
      loadbalancer:
        servers:
          - url: "http://{{lookup('env','platform_01_public_ip')}}:8080/"
          - url: "http://{{lookup('env','platform_02_public_ip')}}:8080/"
          - url: "http://{{lookup('env','platform_03_public_ip')}}:8080/"
        healthCheck:
          path: "/health"
          interval: "1s"
          timeout: "2s"
